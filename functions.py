...
def mouse_click(screen, settings, background_grid,
                drawing, buttons_group, frame, color_indicator,
                eraser_button, reference_grid_button, pointer,
                clear_button, save_button, load_button,
                rectangle_button, rectangles, drawing_button):
	"""Reakcja na kliknięcie na tło myszą."""
	mouse_x,mouse_y = pygame.mouse.get_pos()
	if (pygame.sprite.spritecollideany(pointer, background_grid,
	                                   collided=None)
	    and settings.active_button == "rectangle"):
		...
		create_drawing_element(pointer, background_grid, screen,
		                       settings, drawing)
	...
	elif reference_grid_button.rect.collidepoint(mouse_x, mouse_y):
		...
		if (reference_grid_button.button_on
		    and settings.enable_changing_grid):
			#Wyłączenie siatki.
			reference_grid_button.button_on = False
			settings.enable_changing_grid = False		
		elif (reference_grid_button.button_on == False
		      and settings.enable_changing_grid):
			#Włączenie siatki.
		...
		with open("picture.json", "r") as drawing_load:
			...
			for drawing_element_data in drawing_data:
				drawing_element = DrawingElement(
				    screen, settings, drawing_element_data[1])
				drawing_element.rect.center = drawing_element_data[0]
				drawing.add(drawing_element)
	elif rectangle_button.rect.collidepoint(mouse_x, mouse_y):
		#Włączenie trybu rysowania prostokątów.
		settings.active_button = "rectangle"
	elif drawing_button.rect.collidepoint(mouse_x, mouse_y):
...
def erase_drawing_element(pointer, drawing):
	...
	existing_drawing_element = pygame.sprite.spritecollideany(
	    pointer, drawing, collided=None)
...
def drawing_rectangle(background_grid, rectangles, drawing, screen,
	...
	covered_part = pygame.sprite.groupcollide(background_grid,
	                                          rectangles, False, False,
	                                          collided=None)
...
def update_screen(screen, drawing, frame, buttons_group,
